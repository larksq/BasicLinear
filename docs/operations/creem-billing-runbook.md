# BasicLinear Creem billing

## Implementation and validation — 2026-09-14

The hosted backend supports `BASICLINEAR_PAYMENT_MODE=creem`. Existing monthly ($2 per active seat) and annual ($12 per active seat) USD plans are preserved. This follows talk2format's server-only Creem credentials, raw-body webhook signature verification, and real `www.creem.io/checkout/prod_…/ch_…` URL handling. BasicLinear uses recurring subscriptions rather than talk2format's prepaid packs.
...
Production uses Vercel project `basiclinear-online` and the verified custom
domain `basiclinear.qiaosun.me`. On September 22, 2026, the authoritative
CNAME was published, Vercel served the BasicLinear homepage over HTTPS, and the
production Cloud Run public origin and budget-notification audience were updated
to that canonical URL. Previous public links redirect to BasicLinear.
...
| Setting | Value |
| --- | --- |
| `BASICLINEAR_PAYMENT_MODE` | `creem` |
| `BASICLINEAR_CREEM_API_KEY` | Secret Manager reference; `creem_test_…` in development, live key in production |
| `BASICLINEAR_CREEM_WEBHOOK_SECRET` | Separate signing secret for the environment's webhook |
| `BASICLINEAR_CREEM_MONTHLY_PRODUCT_ID` | Recurring USD 200 cents, every-month |
| `BASICLINEAR_CREEM_ANNUAL_PRODUCT_ID` | Recurring USD 1200 cents, every-year |
| `BASICLINEAR_HOSTED_PUBLIC_ORIGIN` | Exact frontend origin, also present in `BASICLINEAR_HOSTED_ORIGINS` |
...
1. Obtain the test API key and signing secret through a secure local file or scoped Secret Manager references. Create the two BasicLinear test products; do not reuse talk2format's prepaid products.
...
Creem documents checkout retrieval by ID, but no checkout search or expiration endpoint. BasicLinear therefore refuses to replace an unresolved payable checkout, and refuses automatic recovery of an unknown timed-out attempt after its 24-hour window. An operator must reconcile such a checkout with Creem; do not clear the workspace checkout lock while a charge is possible. The 24-hour application return-link window is not proof that Creem has expired a checkout.
...
- API secret: `openlinear-dev-larksq/basiclinear-creem-test-api-key`, version 1
- Webhook secret: `openlinear-dev-larksq/basiclinear-creem-test-webhook-secret`, version 1
- Completed isolated test checkout: `ch_67FVXhkDKSPydIrJ22HMzo`
- Canceled isolated test subscription: `sub_KaDeSdlbHzdyHUcBCoAa`

Next: publish `basiclinear CNAME cname.vercel-dns.com.`, verify custom-domain
HTTPS and authenticated workspace access, then finish a trusted workspace
payment test once operations activation evidence is available. Separate live
products and credentials are deployed as recorded below.


## Production deployment — 2026-09-14

- Backend revision: `openlinear-hosted-api-00012-67d`, serving 100% traffic.
- Image: `us-central1-docker.pkg.dev/openlinear-prod-larksq/basiclinear/hosted-api:creem-20260914-durable`.
...
- Secret Manager: `basiclinear-creem-api-key:1` and `basiclinear-creem-webhook-secret:1`, in `openlinear-prod-larksq`, accessible to the production runtime service account. Dedicated Creem key has checkout, product, and subscription read/write scopes.
