...
Measure a reproducible, non-destructive Linear reference baseline for eight approved project, milestone, issue, view, search, and detail tasks, then exercise behaviorally corresponding OpenLinear tasks through the same Chrome automation harness. This protocol supplies a reference worksheet and an engineering diagnostic; it does not replace the eligible-user O-002 observation window.
...
- OpenLinear uses the synthetic `ct12-acceptance-fixture-v1` workspace at `http://localhost:4175`.
...
| OpenLinear build | Unborn, uncommitted repository state; exact production bundle hashes are in `matched-comparison.json` |
...
6. Use the median of three attempts per task. O-002's diagnostic is the median across tasks of `100 * OpenLinear task median / Linear task median - 100`.
...
T05 is analogous rather than structurally identical: Linear exposes a workspace Views entry, while OpenLinear exposes the seeded saved issue view. T03 permits zero OpenLinear interactions because milestones are already in the default project overview. These differences are retained as limitations rather than hidden through normalization.

## Retained Evidence

- `reference-baseline.json`: anonymized Linear samples, medians, interactions, and limitations.
- `matched-comparison.json`: OpenLinear samples and same-harness diagnostic comparison.
