...
The desired behavior change is that a new owner can sign in, create a first task, invite one teammate, assign work, and receive a teammate comment without leaving BasicLinear. A second behavior change is that an external client can perform the same bounded PM actions through documented contracts.
