...
|---|---|---|
| `problem.target-user-specific` | Pass | Two-to-ten-person product teams led by a technical founder or product lead. |
| `problem.user-role` | Pass | Owner administers; invited member executes and comments. |
| `problem.current-workflow` | Pass | Existing local owner workflow cannot safely support remote collaborators. |
| `problem.partial-tool-alternatives` | Pass | Hosted trackers, chat, and unsupported remote access are explicit alternatives. |
