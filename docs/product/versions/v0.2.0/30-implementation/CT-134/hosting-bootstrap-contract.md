...
The Vite build has two independent HTML entries. `index.html` continues to boot the local product and its SQLite-backed local-owner session. `hosted.html` boots OpenLinear Online and never calls the local-owner endpoint. Firebase Hosting serves `apps/web/dist` but explicitly excludes the local `index.html` from deployment, preserves generated static assets, sends only `/api/**`, `/mcp`, and `/oauth/**` to the named `openlinear-hosted-api` Cloud Run service, and falls back product routes to `hosted.html`.
...
The browser initializes Firebase from the four public `VITE_FIREBASE_*` identifiers and starts `signInWithPopup` with `GoogleAuthProvider`. It stores Firebase browser persistence, not an OpenLinear password. The Firebase ID token is sent only in the same-origin `Authorization` header to `POST /api/v1/hosted/bootstrap`; it is never placed in a URL, response, checkpoint, or log.
