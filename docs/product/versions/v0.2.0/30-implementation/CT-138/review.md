...
Review 7 corrects the evidence universe without weakening it. `npm query .workspace` enumerates 11 workspaces. Ten declare `typecheck` and the root `npm run typecheck` executes all ten: `@openlinear/api`, `@openlinear/hosted-service`, `@openlinear/web`, `@openlinear/contracts`, `@openlinear/db`, `@openlinear/domain`, `@openlinear/hosted`, `@openlinear/operator`, `@openlinear/test-fixtures`, and `@openlinear/ui`. `@openlinear/config` exports only `typescript.json`; it has no executable TypeScript surface or typecheck script. A fresh primary run passed all 10 declared scripts.
