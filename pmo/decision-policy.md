...
OpenLinear distinguishes product, technical, quality, operational, legal/brand, and routine execution decisions. Product decisions change the target segment, problem, scope, outcome, or release promise. Technical decisions change architecture, data authority, security posture, migration model, or supported deployment. Quality decisions change acceptance methods or thresholds. Operational decisions change rollout, backup, support, or incident response. Legal/brand decisions change license, name, identity, attribution, or reference-use boundaries. Routine execution decisions select an implementation approach inside accepted contracts.

The Heavy approval posture applies to problem framing, first-release scope, outcome contracts, acceptance thresholds, security boundary, destructive migrations, license, public name, and release. Routine reversible work proceeds without repeated approval when it stays inside accepted issues and guardrails.

## Authority

The sponsor acts as product owner and accepts product scope, outcomes, release identity, and release readiness. The technical lead accepts architecture and migration proposals after security and test review. The security owner may block a candidate that violates authorization, secret, content, or recovery controls. The test owner decides whether evidence satisfies the accepted method but cannot redefine the product target. A qualified legal or brand reviewer accepts public naming, license posture, asset provenance, and clean-room release boundaries.
...
A decision record must cite stable evidence references, distinguish verified fact from inference and assumption, and explain why rejected alternatives do not satisfy current constraints. Vendor claims are not accepted as independent performance evidence. Private screenshots cannot be attached to public issues or tracked files. As of 2026-08-20, CT-3 also pauses new authenticated competitor observation and quarantines prior authenticated-reference aggregates from publication pending qualified review. Test results identify source revision, environment, dataset, command or method, and artifact location.

If evidence is missing, the record names validation work, accountable role, due date, and decision point. Silence is not acceptance. Access to Chrome, Google, GitHub, or any other account is capability, not authority for external mutation.

## Cadence

Stage gates are the principal cadence. Discovery reviews problem, users, alternatives, feasibility, requirements, outcomes, and gaps. Sprint Planning reviews PRD, architecture, UX, risk, acceptance, test strategy, and the issue graph. Implementation reviews bounded slices as they complete. Testing performs independent verification. Outcome Review observes only after valid windows. Retrospective reviews process and AI-session evidence.

Within a stage, weekly product and technical reviews reconcile evidence, scope, and risk. Security and visual baseline changes are reviewed when proposed rather than deferred to release. Urgent incidents can trigger immediate decisions, but the receipt is recorded after containment.
