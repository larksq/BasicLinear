...
| Urgent | 6 | Technical source-release audit | CT-13 | Done; the current release audit checks source identity, notices, provenance, private paths, secrets, reproducible inputs, and the clean-host result. Refresh the generated evidence for each candidate. |
| Urgent | 7 | Release-candidate outcome review | CT-14 | Planned; outcome measurement is separate from technical release readiness. CT-2 is evidence, not an active blocker. |
