...
export function readAllowedOrigins(value: string | undefined): string[] {
  ...
  return value.split(',').map((entry) => {
    ...
    if (
      ...
      throw new Error('OPENLINEAR_HOSTED_ORIGINS must contain comma-separated origins.');
    }
...
export function readInvitationSecret(value: string | undefined): string {
  ...
  if (length < 32 || length > 512) {
    throw new Error('OPENLINEAR_INVITATION_SECRET must contain between 32 and 512 bytes.');
  }
  return secret;
}
...
export function readCollaborationSecret(value: string | undefined): string {
  ...
  const length = Buffer.byteLength(secret, 'utf8');
  if (length < 32 || length > 512) {
    throw new Error('OPENLINEAR_COLLABORATION_SECRET must contain between 32 and 512 bytes.');
  }
  return secret;
}
...
export function readBillingSecret(value: string | undefined): string {
  ...
  const length = Buffer.byteLength(secret, 'utf8');
  if (length < 32 || length > 512) {
    throw new Error('OPENLINEAR_BILLING_SECRET must contain between 32 and 512 bytes.');
  }
  return secret;
}
...
export function readPersonalTokenSecret(value: string | undefined): string {
  return readBoundedSecret(value, 'OPENLINEAR_PERSONAL_TOKEN_SECRET');
}

export function readProjectManagementSecret(value: string | undefined): string {
  return readBoundedSecret(value, 'OPENLINEAR_PM_SECRET');
}

export function readRestCursorSecret(value: string | undefined): string {
  return readBoundedSecret(value, 'OPENLINEAR_REST_CURSOR_SECRET');
}

export function readMcpOAuthSecret(value: string | undefined): string {
  return readBoundedSecret(value, 'OPENLINEAR_MCP_OAUTH_SECRET');
}

export function readOperationsSecret(value: string | undefined): string {
  return readBoundedSecret(value, 'OPENLINEAR_OPERATIONS_SECRET');
}
...
export function readHostedEnvironment(value: string | undefined): 'uat' | 'production' {
  ...
  if (environment !== 'uat' && environment !== 'production') {
    throw new Error('OPENLINEAR_HOSTED_ENVIRONMENT must be uat or production.');
  }
  return environment;
}
...
export function readHostedPaymentMode(value: string | undefined): HostedPaymentMode {
  ...
  if (mode !== 'stripe' && mode !== 'creem' && mode !== 'verification') {
    throw new Error('OPENLINEAR_PAYMENT_MODE must be stripe, creem or verification.');
  }
  return mode;
}
...
export function readVerificationAccessEmails(value: string | undefined): string[] {
  ...
  if (emails.length > 20 || new Set(emails).size !== emails.length
    || emails.some((email) => email.length > 254
      || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/u.test(email))) {
    throw new Error('OPENLINEAR_VERIFICATION_ACCESS_EMAILS must contain distinct email addresses.');
  }
  return emails;
}
...
export function readHostedDeploymentEnvironment(
  ...
  if (environment !== 'development' && environment !== 'production') {
    throw new Error('OPENLINEAR_DEPLOYMENT_ENVIRONMENT must be development or production.');
  }
  return environment;
}
...
export function readFirestoreDatabaseId(value: string | undefined): string {
  ...
  if (!/^[a-z][a-z0-9-]{2,61}[a-z0-9]$/u.test(databaseId)) {
    throw new Error('OPENLINEAR_FIRESTORE_DATABASE_ID must be (default) or a canonical database ID.');
  }
  return databaseId;
}
...
export function hostedStripeMode(secret: string): HostedStripeMode {
  ...
  throw new Error('OPENLINEAR_STRIPE_SECRET must identify one Stripe test or live account mode.');
}
...
export function assertHostedDeploymentBinding(input: Readonly<{
  ...
  if (input.firebaseProjectId !== expectedProjectId) {
    throw new Error('OPENLINEAR_FIREBASE_PROJECT_ID does not match the selected deployment environment.');
  }
  ...
  if (input.hostedEnvironment !== expectedHostedEnvironment) {
    throw new Error('OPENLINEAR_HOSTED_ENVIRONMENT does not match the selected deployment environment.');
  }
...
export function readBudgetPushAudience(value: string | undefined): string {
  ...
  try {
    ...
    throw new Error('OPENLINEAR_BUDGET_PUSH_AUDIENCE must be the exact budget push URL.');
  }
  const secure = parsed.protocol === 'https:';
  const loopbackHttp = parsed.protocol === 'http:' && loopbackHosts.has(parsed.hostname);
...
export function readBudgetPushServiceAccount(value: string | undefined): string {
  ...
  if (email.length > 254
    ...
    throw new Error('OPENLINEAR_BUDGET_PUSH_SERVICE_ACCOUNT must be an exact service-account email.');
  }
  return email;
}
...
export function assertBudgetPushAudienceOrigin(audience: string, publicOrigin: string): void {
  if (new URL(audience).origin !== publicOrigin) {
    throw new Error('OPENLINEAR_BUDGET_PUSH_AUDIENCE must use OPENLINEAR_HOSTED_PUBLIC_ORIGIN.');
  }
}
...
export function readStripeSecret(value: string | undefined): string {
  ...
  if (!/^sk_(?:test|live)_[A-Za-z0-9_]{16,240}$/u.test(secret)) {
    throw new Error('OPENLINEAR_STRIPE_SECRET must be a server-side Stripe secret key.');
  }
  return secret;
}
...
export function readStripeWebhookSecret(value: string | undefined): string {
  ...
  if (!/^whsec_[A-Za-z0-9_]{16,240}$/u.test(secret)) {
    throw new Error('OPENLINEAR_STRIPE_WEBHOOK_SECRET must be a Stripe endpoint secret.');
  }
  return secret;
}
...
export function readHostedPublicOrigin(value: string | undefined): string {
  ...
  try {
    ...
    throw new Error('OPENLINEAR_HOSTED_PUBLIC_ORIGIN must be one exact origin.');
  }
  const secure = parsed.protocol === 'https:';
  const loopbackHttp = parsed.protocol === 'http:' && loopbackHosts.has(parsed.hostname);
  if ((!secure && !loopbackHttp) || raw !== parsed.origin) {
    throw new Error('OPENLINEAR_HOSTED_PUBLIC_ORIGIN must be one exact HTTPS or loopback origin.');
  }
  return parsed.origin;
}
...
export function readCreemApiKey(value: string | undefined): string {
  ...
  if (!/^creem_[A-Za-z0-9_-]{16,240}$/.test(key)) throw new Error('OPENLINEAR_CREEM_API_KEY must be a server-side Creem key.');
  return key;
}
export function readCreemWebhookSecret(value: string | undefined): string {
  const secret = value?.trim() ?? '';
  if (secret.length < 16 || secret.length > 256 || /\s/.test(secret)) throw new Error('OPENLINEAR_CREEM_WEBHOOK_SECRET is required.');
  return secret;
}
