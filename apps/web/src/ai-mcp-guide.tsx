...
const ONLINE_MCP_URL = 'https://basiclinear.qiaosun.me/mcp';
...
function ClientCommands({
  ...
  return (
    <section className="ai-mcp-client-card" aria-labelledby={`ai-mcp-${client === 'Codex' ? 'codex' : 'claude'}`}>
      ...
      {client === 'Claude Code' && <p className="ai-mcp-client-note">CLI login requires Claude Code 2.1.186 or later. On older versions, open Claude Code, run <code>/mcp</code>, and select BasicLinear to sign in.</p>}
    </section>
  );
}
...
export function AiMcpGuide({serverUrl, surface, collapsed = false}: AiMcpGuideProps) {
  ...
  const codexCommands = `codex mcp add basiclinear --url ${mcpUrl}\ncodex mcp login basiclinear\ncodex mcp list`;
  const claudeCommands = `claude mcp add --transport http basiclinear ${mcpUrl}\nclaude mcp login basiclinear\nclaude mcp list`;
      ...
      >
        <header className="ai-mcp-guide-header">
          <div>
            ...
            <p id={descriptionId}>Connect through BasicLinear MCP, approve only the workspace and scopes you need, then operate issues in plain language.</p>
          </div>
        ...
        <div className="ai-mcp-guide-body">
          {surface === 'local' ? (
            <div className="ai-mcp-boundary-note">
              <ShieldCheck size={17} />
              <div><strong>Your local workspace stays local.</strong><span>These commands connect to <a href="https://basiclinear.qiaosun.me/?app" target="_blank" rel="noreferrer">BasicLinear Online</a>; it does not upload or synchronize this SQLite workspace. For a self-hosted workspace, use the MCP endpoint shown in that workspace’s AI guide.</span></div>
            </div>
          ) : (
          ...
          <section className="ai-mcp-guide-step" aria-labelledby={`${titleId}-connect`}>
            ...
            <div>
              ...
              <p>{surface === 'local' ? 'Connect to BasicLinear Online, then choose the hosted workspace you want AI to operate.' : 'Add this workspace as a remote Streamable HTTP MCP server.'}</p>
          ...
          <section className="ai-mcp-guide-step" aria-labelledby={`${titleId}-operate`}>
            ...
            <div>
              ...
              <ul className="ai-mcp-prompt-list">
                <li><code>Use BasicLinear to list the workspaces I can access. Do not change anything.</code></li>
          ...
          <footer className="ai-mcp-guide-safety">
            <ShieldCheck size={16} />
            <p><strong>Same permissions, whichever client you use.</strong> BasicLinear checks membership, scopes, revisions, and retry safety on every MCP call. Ask for confirmation before removing a member, revoking an invitation, or deleting a comment.</p>
          </footer>
