...
const repositoryUrl = import.meta.env.VITE_BASICLINEAR_REPOSITORY_URL?.trim();
...
function ProductPreview() {
  ...
  return <div className="hp-preview" aria-label="Interactive product preview with sample issues">
    <aside className="hp-preview-sidebar">
      <div className="hp-preview-brand"><BrandMark /> BasicLinear <ChevronDown size={12} /></div>
      ...
      <div className="hp-preview-nav"><GitBranch size={15} /> BasicLinear launch</div>
      <div className="hp-preview-local"><Database size={13} /> A space for your next idea</div>
    </aside>
    <div className="hp-preview-main">
      <div className="hp-preview-top"><span>BasicLinear launch <span className="hp-preview-slash">/</span> <strong>Issues</strong></span><span className="hp-demo-badge">Interactive preview</span></div>
...
const faqs = [
  { q: 'Is this made by Linear?', a: 'No. BasicLinear is an independent, open-source project inspired by Linear’s focused approach to product management. It is not affiliated with, endorsed by, or an official version of Linear.' },
  ...
  { q: 'Is BasicLinear free?', a: 'The local edition is free to run and licensed under AGPL-3.0-only. The online edition is a separate hosted service; its trial and plan details are shown in the app. Running locally does not require a hosted subscription.' },
  ...
  { q: 'Can I change the code?', a: 'Yes. BasicLinear is licensed under AGPL-3.0-only. You can inspect and modify the source under that license. Read the included license for the conditions that apply when distributing changes or offering a modified version over a network.' },
];
  ...
  return <div className="hp" onKeyDown={event => { if (event.key === 'Escape' && menuOpen) { setMenuOpen(false); menuButton.current?.focus(); } }}>
    ...
    <header className="hp-header">
      <div className="hp-header-inner">
        <a className="hp-brand" href="/" aria-label="BasicLinear home"><BrandMark /><span>BasicLinear</span></a>
    ...
    <main id="main-content" tabIndex={-1}>
      ...
      <section id="how-it-works" className="hp-container hp-section" aria-labelledby="hp-workflow-title">
        <div className="hp-section-heading"><div><p className="hp-eyebrow">02 / From idea to done</p><h2 id="hp-workflow-title">A simple rhythm.<br />Every day.</h2></div><p>Start with the outcome. Break it down. Move one useful thing forward. BasicLinear keeps the thread.</p></div>
      ...
      <section id="open-source" className="hp-container hp-section hp-open" aria-labelledby="hp-open-title">
        <div><p className="hp-eyebrow"><GitBranch size={13} /> 03 / Open by design</p><h2 id="hp-open-title">Your workflow.<br />Your code.<br /><span>Your call.</span></h2><p>Good tools should leave room for your way of working. Run BasicLinear on your computer, explore the source, and make it your own.</p><a className="hp-text-link" href={sourceUrl ?? '/basiclinear-license.txt'}>{sourceUrl ? 'Explore the source on GitHub' : 'Read the AGPL-3.0 license'} <ArrowUpRight size={15} /></a>{!sourceUrl && <p className="hp-source-note">The source repository is not public yet. You can use the online app now.</p>}</div>
    ...
    <footer className="hp-container hp-footer"><div className="hp-footer-top"><a className="hp-brand" href="/"><BrandMark />BasicLinear</a><nav aria-label="Footer navigation"><a href="#product">Product</a><a href="#getting-started">Setup guide</a><a href="/basiclinear-license.txt">License</a><a href="#open-source">Open source</a></nav></div><div className="hp-footer-bottom"><p>An independent, open-source project inspired by Linear.<br />Not affiliated with or endorsed by Linear.</p><span>Independent project. AGPL-3.0-only.</span></div></footer>
  </div>;
}
