  ...
  return <div className="hp" onKeyDown={event => { if (event.key === 'Escape' && menuOpen) { setMenuOpen(false); menuButton.current?.focus(); } }}>
    ...
    <main id="main-content" tabIndex={-1}>
      ...
      <section id="open-source" className="hp-container hp-section hp-open" aria-labelledby="hp-open-title">
        <div><p className="hp-eyebrow"><GitBranch size={13} /> 03 / Open by design</p><h2 id="hp-open-title">Your workflow.<br />Your code.<br /><span>Your call.</span></h2><p>Good tools should leave room for your way of working. Run OpenLinear on your computer, explore the source, and make it your own.</p><a className="hp-text-link" href={sourceUrl ?? '/openlinear-license.txt'}>{sourceUrl ? 'Explore the source on GitHub' : 'Read the AGPL-3.0 license'} <ArrowUpRight size={15} /></a>{!sourceUrl && <p className="hp-source-note">Public repository link coming soon.</p>}</div>
